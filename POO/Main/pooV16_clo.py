...
modifs effectuées : 
    ajout de compteurs
    modification de la classe objectifs
    modification des fonctionx exploser bonus et elements : incrémentation du compteur correspondant
    le compteur d'étoiles dans les puits est à incrémenter à la fin de chaque tour ?
                    --> implémenté dans la fonction set_dans_puits
...
#arbreXML = ET.parse("C:\\Users\\strel\\OneDrive\\Desktop\Arts et Métiers\\1ère Année\\GIE 1\\Projet POO\\Niveaux\\Niveau3 - Introduction des cellules vides.xml")
...
class Objectif():
    ...
    def affichage(self):
        if self._cible in la_map.sa_liste_couleurs:
            ...
            return "Objectif : Activer {} bonus.".format(self.sa_quantite)
        else:
            return "Objectif : Disparition de toutes les étoiles du niveau"
    def __repr__(self):
...
##TODO compteur qui augmente lorsqu'on met une étoile dans un puits
...
#Chargement de la Map
    ...
    for objectif_xml in tronc[1]:
        la_cible = objectif_xml.get("Cible")[1:]        #on zocque les espaces au début
...
class Interface():
    def __init__(self,ma_map):        
        ...
        remplissage_grille()
        #la_map.sa_grille.test_grille()         #tester au début, mais ne parche pas pour le moment...
            
        l+=ma_map.ses_dimensions[0]
        
        #Affichage des coordonnées des cases (pour le joueur)
        for i in range(la_map.ses_dimensions[0]):
        ...
        for j in range(la_map.ses_dimensions[1]):
        ...
        #Fenêtre contextuelle : "coup non valide", ou qui dit les actions qui ont été effectuées, en 2+decalage
        
        
        #Affichage de l'avancement des objectifs (faire qqch de dynamique qui s'adapte au xobjectifs de la_map)
        TK.Label(self.fenetre, text="Elements détruits:").grid(row=l, column=0, columnspan=3, sticky = 'w n s', padx = 5, pady = 5)
        TK.Label(self.fenetre, text="objectifs d'éléments").grid(row=l, column=3, columnspan=4, sticky = 'w e n s', padx = 5, pady = 5)
        l+=1
        
        TK.Label(self.fenetre, text="Boni activés:").grid(row=l, column=0, columnspan=3, sticky = 'w n s', padx = 5, pady = 5)
        TK.Label(self.fenetre, text="Boni activés").grid(row=l, column=3, columnspan=4, sticky = 'w e n s', padx = 5, pady = 5)
        l+=1
        
        TK.Label(self.fenetre, text="Etoiles dans un puits:").grid(row=l, column=0, columnspan = 3, sticky= "w n s", padx = 5, pady = 5)
        TK.Label(self.fenetre, text="nb d'étoiles dans un puits").grid(row=l, column=3, columnspan = 4, sticky= "w e n s", padx = 5, pady = 5)
        l+=1
